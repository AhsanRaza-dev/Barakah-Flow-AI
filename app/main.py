...
@app.post("/api/ask")
def ask_barakah_ai(request: AskRequest, background_tasks: BackgroundTasks, token: str = Depends(verify_token)):
    ...
    if db:
        try:
            ...
            if cached_result:
                print("⚡ CACHE HIT! Returning saved response in 0.1s...")
                return {
                    "status": "success",
                    "source": "cache",
                    "madhab_applied": selected_madhab,
    ...
    # 🟢 Triple-Threat Multi-Lingual Query Generation
    ...
    2. Resolve pronouns (e.g., replace "is main" with the actual topic like "Witr").
    3. Translate the resolved query into clear English (vital for semantic search).
    4. Provide the core Classical Arabic terminology (e.g., "دعاء القنوت", "نص الدعاء اللهم إنا نستعينك").
    ...
    # 🟢 Step B: RAM-Optimized Fast Search
    books_context = ""
    db = get_db_connection()
    if db:
        try:
    ...
    # 🟢 STEP C: Final AI Generation & NEW Comparator Logic
    comparator_logic = ""
    if selected_madhab == "Any":
        comparator_logic = """
    ...
    if final_answer:
        # 🟢 Post-Processing: Cache Save
        db = get_db_connection()
        if db:
            try:
                cursor = db.cursor()
                cursor.execute("INSERT INTO response_cache (query, madhab, response) VALUES (%s, %s, %s)", (user_query, selected_madhab, final_answer))
                db.commit()
            except Exception as e:
                pass
            finally:
        ...
        # 🟢 THE SPEED HACK: Supabase logging offloaded to background
        background_tasks.add_task(save_to_supabase_bg, request.user_id, request.session_id, user_query, final_answer)
